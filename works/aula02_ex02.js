...
function rotateCylinder()
{
  ...
  c1.matrix.multiply(mat4.makeRotationY(angle[3])); // RY
  ...
  c2.matrix.multiply(mat4.makeRotationY(angle[3])); // RY
  c2.matrix.multiply(mat4.makeTranslation(0.0, 1.0, 0.0)); // T2

  // Just need to translate the sphere to the right position
  s3.matrix.multiply(mat4.makeTranslation(0.0, 1.0, 0.0)); 
  s3.matrix.multiply(mat4.makeRotationZ(angle[1])); 
  s3.matrix.multiply(mat4.makeTranslation(0.0, 2.0, 0.0));  

  
  // Will execute T3 and then R3
  c3.matrix.multiply(mat4.makeRotationZ(angle[2])); // R3
  c3.matrix.multiply(mat4.makeRotationY(angle[3])); // RY
...
function buildInterface()
{
  var controls = new function ()
  {
    ...
    this.rotate = function(){
      ...
      angle[3] = degreesToRadians(this.joint12);
      rotateCylinder();
    };
  };
  ...
  gui.add(controls, 'joint1', 0, 360)
    ...
    .name("First Joint Z");
  gui.add(controls, 'joint12', 0, 360)
    .onChange(function() { controls.rotate() })
    .name("First Joint Y");
